describe('run/requirejsInstrumentor', function() {
    var testframe, config, global, utils, htmlInstrumentor,fileLoader,
    ...
    beforeEach(function() {
        ...
        testframe = {
            win: jasmine.createSpy().andReturn(win),
        ...
        var modules = uitest.require({
            ...
            global: global
        }, ["run/requirejsInstrumentor", "utils", "run/htmlInstrumentor"]);
        utils = modules.utils;
        htmlInstrumentor = modules["run/htmlInstrumentor"];
    ...
    function processHtml(baseUrl, html) {
        var resultCb = jasmine.createSpy('result');
        htmlInstrumentor.processHtml(baseUrl,resultCb);
        fileLoader.mostRecentCall.args[1](undefined, html);
        return resultCb.mostRecentCall.args[1];
    }
    it('should append an inline script after requirejs', function() {
        var input = '<html><script src="require.js"></script></html>';
        expect(processHtml('/someBase/someUrl', input)).toBe('<html><script src="/someBase/require.js"></script><script>##rc(window);</script></html>');
    });
    it('should call the original require-callback with the original args', function() {
        var reqConfig = {};

        require.config = reqConfig;
        processHtml('/someBase/someUrl', HTML);
        testframe.createRemoteCallExpression.argsForCall[0][0](win);
    ...
    describe('load interceptor', function() {
        ...
        function exec(url) {
            ...
            win.location.href = '/someBase/someUrl';
            processHtml(win.location.href, HTML);
        ...
        it('should call the original require.load url', function() {
            exec("someUrl");
            expect(requireLoad).toHaveBeenCalledWith(loadContext, someModuleName, "/someBase/someUrl");
        });
        it('should trigger instrumentScript event', function() {

        });
        // TODO check event instrumentScript is fired
        // with abs url made using location.href!
        /*TODO
